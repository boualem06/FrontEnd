...
function UsersManagement({users, setUsers , userAvisualiser , setUserAvisualiser }) {
  ...
  for (let i = 0; i < filterUsers.length; i++) {
    UsersList.push(
      <tr>
        <td className="p-1">
          {<span className="bg-slate-700 rounded-xl">{i + 1}</span>}
        </td>
        <td className="p-1">{filterUsers[i].name}</td>
        <td className="p-1">{filterUsers[i].email}</td>
        <td className="p-1">{filterUsers[i].phone}</td>
        <td className="p-1">{filterUsers[i].role  == 0 ? "admin " : filterUsers[i].role  == 1 ? "gestionnaire" : "simple"}</td>
        <td className="p-1">
          {
            <div className="space-x-2">
              <span className="bg-violet-800 p-1 rounded-md hover:bg-violet-600">
                <button onClick={async ()=>{console.log("user a informer " ,filterUsers[i] ) ; 
              ...
              <span className="bg-green-700 p-1 rounded-md hover:bg-green-500">
                <button onClick={async()=> { await setUserAvisualiser(filterUsers[i]) }}>
                  {" "}
                  <Link to={"/ModiferUser" } >
                  {<FaWrench />}
                  </Link>
  ...
  return (
    <div className="p-6">
      {/* <Router>
                       <Route path={"/UserInformation"}> <UserInformations user={filterUsers[i]}></UserInformations></Route>
                  </Router> */}
      {!confirmer && (
        <div>
          {" "}
            ...
            <div className="flex justify-between">
              ...
              <div className="self-center space-x-4">
                ...
                <button className="text-white bg-slate-900 p-2 self-center hover:bg-slate-800 hover:scale-105 rounded-md">
                  {/* <Link to={<CreateUser />}>+ Add User</Link> */}+ Add User
                </button>
              </div>
            </div>
            {UsersList.length > 0 && (
      ...
      {confirmer && (
        <div className="rounded-xl shadow-xl mx-auto mt-12 lg:w-1/3 sm:w-1/2 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500  ">
