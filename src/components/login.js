...
//  console.log((useContext(LoginContext)).isAuth) ;
  ...
  const handleChange = (e) =>
    ...
    return (
      ...
      // </div> 
        <div className=" bg-cover relative flex md:justify-center  md:items-center" style={{ backgroundImage: `url(${background})`,height:"100vh" }} >
        ...
        <div className="  flex md:w-4/5 lg:w-3/12 lg:w-1/2">
          ...
          <div style={{ borderBottomRightRadius:"15px",borderTopRightRadius:"15px"}} className=" shadow-xl bg-white shadow-gray-500/75  py-4  md:w-1/2  relative     ">  
          <div   className="flex justify-center mb-14"> 
            <h1 className="font-bold text-4xl ">Connexion</h1>

          </div>
          {!valide && <p className=" w-full  font-bold text-center text-red-500 m-2">{errorsOrId.errors}</p>}
        
          {!props.isAuth && <div className=" pb-12 flex flex-col items-center  ">
          ...
          <input value={formValues.email}  type="email" id="email" name="email" onChange={handleChange }  className=" px-4 h-12   w-9/12 bg-gray-300 border-none rounded-lg" />
          <p className=" mt-1 font-bold text-red-500">{formErrors.email}</p>
          </div>}

         {!props.isAuth &&  <div className=" pb-10 flex flex-col items-center  ">
         {/* {!isAuth &&  <div className=" pb-10 flex flex-col items-center  "> */}
            <div className="text-left w-9/12 pb-4">
          <label className=" font-bold py-2" htmlFor="password">Mot de passe</label>
            </div>
          <input value={formValues.password} onChange={handleChange}  type="password" id="password" name="password" className=" px-4 h-12    w-9/12 bg-gray-300 border-none rounded-lg" />
          <p className="  px-4  mt-1 font-bold text-red-500">{formErrors.password}</p>
          </div>
          }
          {!props.isAuth && 
          ...
          <button onClick={handleSubmit} className=" hover:bg-blue-700 bg-blue-600 px-10 text-white font-bold text-lg py-2 w-auto border-none rounded-full ">
            {
               !loading ? <span>connecter </span> : <div class="loadingio-spinner-spin-guwuy00vjh"><div class="ldio-kp6g3e7ejdo">
          ...
          {/* <h1 className="hover:cursor-pointer hover:text-gray-600  font-bold text-blue-600  pt-2"> Mot de passe oublie ?</h1> */}
            <Link to={"/ForgotPassword"}><h1 className="hover:cursor-pointer hover:text-gray-600  font-bold text-blue-600  pt-2"> Mot de passe oublie ?</h1></Link>
          </div>
          </div>
          }
          {props.isAuth && 
          ...
          <div className=" grid gap-16 h-2/5 mt-24 flex flex-col justify-center"> 
            <h1 className=" hover:text-green-800 text-xl  font-bold text-green-600  pt-2">Vous etes connectes </h1>
            <Link to={"/Home"}><button className="mt-8 hover:bg-blue-700 bg-blue-600 px-10 text-white font-bold text-lg py-2 w-auto border-none rounded-full">Aller a l'Acceuil</button></Link>
          </div>}
